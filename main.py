...
async def birthdayloop():
	...
	if datetime.time(hour=8, minute=0).strftime('%H:%M') == datetime.datetime.now().strftime('%H:%M'):
		try:
			config = sjson  # config laden
			for server in config:
				try:
					try:
						roleid = config[server]["bdayrole"]
					...
					# Remove past Birthdays
					rm_bday_members = guild.get_role(roleid)
					print(rm_bday_members.members, "hier sind die rollen")
					for member in rm_bday_members.members:
						if member in birthdayMembers:
							continue
						await member.remove_roles(rm_bday_members, reason="Birthday is over")
